...
.ch_bg
{
	...
	<section id="" class="ch_bg cause_section">
				...
				<div class="toogle" style="padding-bottom:100px;">
					...
					?>
								...
								<section class="toggle">
									...
									<label>
										<a href="<?=base_url()?>causes/details/<?=$causes_data->id?>" style="color: black;">
										<?=$causes_data->title?>
										...
										<a href="<?=base_url()?>causes/cause_modify/<?=$causes_data->id?>" onclick="javascript:check=confirm('Are you sure to perform this action?');if(check==false) return false;" style="float: right;margin-left: 10px;color: red;">Modify</a>
										...
										<span class="publish_data">
											<?php
												...
												{
													echo "(Waiting for Approval from Admin - ".date("M j, Y",strtotime($causes_data->edate)).")";
												}
		...
		<div class="container">
            <div class="page_head_text">
                <h2 class="about_tit text-center wow fadeInDown">My Joined Causes</h2>
            </div>
			<div class="row">
				...
				<div class="toogle" style="padding-bottom:100px;">
					...
					<?php
						if($join_causes && count($join_causes) > 0)
						{
							foreach($join_causes as $joined_causes_data)
							{
					?>
								...
								<section class="toggle">
									...
									<label>
										<a href="<?=base_url()?>causes/details/<?=$joined_causes_data->id?>" style="color: black;">
										<?=$joined_causes_data->title?>
										<span class="more_tex">More Detail</span>
										
										<a href="<?=base_url()?>causes/unfollow/<?=$joined_causes_data->cause_member_id?>" onclick="javascript:check=confirm('Are you sure to perform this action?');if(check==false) return false;" style="float: right;margin-left: 10px;color: red;">
											
											<?php
												if($joined_causes_data->request_status=='Active')
												{
													echo "Unfollow";
												}
												else
										...
										<span class="publish_data">
											<?php
												if($joined_causes_data->request_status=='Active')
												{
													echo "(Joined on - ".date("M j, Y",strtotime($joined_causes_data->joined_date)).")";
												}
												else
												{
					...
					<?php
						...
						{
					?>
							<p>You haven't join any cause.</p>
					<?php
						}
						*/
					?>
