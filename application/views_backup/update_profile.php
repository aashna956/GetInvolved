			...
			<form id="register_panel" name="" method="">
				...
				<div class="col-md-9 form_box_all" style="margin-top:21px;">
					 ...
					 <?php
							...
							{
						?>
								<img src="<?=$profile[0]->image?>" id="pfile" name="pfile" /> <a href="javascript:" id="upload_new_picture" onclick="upload_new()">Change Profile Picture</a>
						<?php
							}
							else
							{
						?>
								<img src="<?=base_url()?>images/blog1.jpg" id="pfile" name="pfile" /> <a href="javascript:" id="upload_new_picture" onclick="upload_new()">Upload New Profile Picture</a>
						<?php
							}
						?>
				 ...
				 <div style="text-align:center;">
					<input type="button" onclick="register_process()" class="login_box6" value="Update">
				</div>
			...
			<form id="register_panel1" name="" method="">
				...
				<div class="col-md-9 form_box_all" style="margin-top:21px;">
				 ...
				 <div style="text-align:left;margin-left: 13px;">
					<input type="button" onclick="update_password_process()" class="login_box6" value="Save">
				</div>
   ...
   <div id="openModalcauses" class="mb_elegantModal" style="display: none;">
	...
	<script type="text/javascript">
		...
		function register_process()
		{
			...
			$.ajax({
					...
					success: function(result){
							if(result)
							{
								$(".notification-bar").html('Your Profile Updated sucessfully.'); 
								$(".notification-bar").css('display','block'); 
								setTimeout(function() { $(".notification-bar").fadeOut(1500); }, 5000);
								return false;
							}
							...
							{
								$(".notification-bar").html('Oops something went wrong. try Again!!'); 
								$(".notification-bar").css('display','block'); 
								setTimeout(function() { $(".notification-bar").fadeOut(1500); }, 5000);
								return false;
		...
		function update_password_process()
		{
			...
			if(ccpwd=='')
			{
				$(".notification-bar").html('Please Provide Confirm Password.'); 
				$(".notification-bar").css('display','block'); 
				setTimeout(function() { $(".notification-bar").fadeOut(1500); }, 5000);
				return false;
			}
			...
			if(ccpwd!=cpwd)
			{
				$(".notification-bar").html('Confirm Password mis-matched.'); 
				$(".notification-bar").css('display','block'); 
				setTimeout(function() { $(".notification-bar").fadeOut(1500); }, 5000);
				return false;
			}
			...
			$.ajax({
					...
					success: function(result){
							if(result)
							{
								$('#ccpwd').val('');
								$('#cpwd').val('');
								$(".notification-bar").html('Your Password Changed sucessfully.'); 
								$(".notification-bar").css('display','block'); 
								setTimeout(function() { $(".notification-bar").fadeOut(1500); }, 5000);
								return false;
							}
